    ...
    public GridRiskResponse gridRisk(String region, int year, int month, int hour, int participation) {
        ...
        if (participation > 0) {
            CoreNumbers core = simulationService.core(
                    new SimulationSettings(true, true, participation, "08:00", "19:00", 500, region, month));
            delta = core.riskBefore() - core.riskAfter();
        }
        final double d = delta;

        // 격자별 현재/적용 후 점수
        List<GridRiskDtos.GridRiskPoint> grids = cache.pointsAt(region, year, month, h).stream()
        ...
        for (int i = 0; i < HOURS; i++) {
            double base = round1(cache.averageAt(region, year, month, 0, i));
            current.add(base);
            projected.add(round1(Math.max(0, base - d)));
        }

        double globalRisk = round1(cache.averageAt(region, year, month, 0, h));
        return new GridRiskResponse(
                h,
                globalRisk,
                round1(Math.max(0, globalRisk - d)),
                grids,
                buildBreakdown(region, year, month, h),
                new HourlyRiskCurve(current, projected));
    }

    public GridRiskResponse gridRisk(int hour, int participation) {
        return gridRisk("pangyo", GridRiskCache.DEFAULT_YEAR, 10, hour, participation);
    }

    private RiskBreakdown buildBreakdown(String region, int year, int month, int hour) {
        // 구성요소는 0~1 스케일로 저장돼 있어 0~100 점수로 환산
        double parking = 100 * (W_DEMAND * cache.averageAt(region, year, month, 1, hour)
                + W_SUPPLY * cache.averageAt(region, year, month, 2, hour))
                / (W_DEMAND + W_SUPPLY);
        double environment = 100 * cache.averageAt(region, year, month, 4, hour);
        double traffic = 100 * cache.averageAt(region, year, month, 3, hour);
