...
public interface RiskIndexRepository extends JpaRepository<RiskIndex, Long> {
    ...
    /** 지도 캐시 적재용 한 행: 격자 좌표 + 시간대 + 지수·구성요소 */
    ...
    /**
     * 히트맵 재료 전체(31,344행 = 1,306격자 × 24h)를 격자 좌표와 함께 조회.
     * 기동 시 1회만 실행해 메모리 캐시에 담는다 (GridRiskCache).
     */
    @Query("""
            select r.gridId as gridId, r.hourOfDay as hourOfDay,
